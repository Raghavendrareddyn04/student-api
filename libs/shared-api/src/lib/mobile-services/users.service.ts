...
export class MobileUsersService {
  ...
  // Then update the loginWithOtp method (around line 368)
async loginWithOtp(request, requests): Promise<any> {
  try {
    ...
    if (number_length == 10 && isNumber(number)) {
      ...
      if (users) {
        if (users?.mobile === 9441856314 || users?.mobile === 9876543211) {
          otp = '654321';
        }
      }
      ...
      if (!users) {
        ...
        newUser.mobile = requests.mobile;
        newUser.uuid = requests.uuid || itUuid.v4();
        newUser.email = '';
        newUser.name = '';
        newUser.otp = otp;
        newUser.signinType = SocialAuthTypes.MOBILE;
        newUser.expiration_time = expiration_time;
        newUser.flags = { isActive: true };
      ...
      if ((await users?.flags.isActive) && users?.mobile != 9876543211 && users?.mobile != 9876543210) {
        const { whatsapp } = environment;
        if (whatsapp?.apiUrl && whatsapp?.token) {
          try {
            const payload = {
              phone_number: number,
            ...
            await fetch(`${whatsapp.apiUrl}?token=${whatsapp.token}`, {
              method: 'POST',
              headers: {
        ...
        console.log('OTP not sent (test number or inactive user)');
      }
    ...
    try {
      ...
      if (number_length == 10 && isNumber(number)) {
        ...
        if (!users) {
          ...
          newUser.uuid = requests.uuid || itUuid.v4();
          newUser.email = '';
          newUser.name = requests.name || '';  // Set name from request
          newUser.otp = otp;
          newUser.signinType = SocialAuthTypes.MOBILE;
          newUser.expiration_time = expiration_time;
          newUser.flags = { isActive: true };
      ...
      console.error('Error in loginWithOtpName:', error);
      return { status: false, code: 5000, message: 'Server error', data: [] };
    }
  }
